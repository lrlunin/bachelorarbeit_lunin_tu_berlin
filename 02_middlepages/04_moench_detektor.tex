...
\noindent
Die Detektion der einzelnen Photonen (in der Literatur oft als \emph{engl. single-photon counting} gezeichnet) stellt eine besondere algorithmische Herausforderung dar. Es gibt zahlreiche Algorithmen und es wird ständig nach besseren Lösungen gesucht. Die Effizienz des Einsatzes eines oder anderen Algorithmen hängt von der Detektorauflösung, Photonenfluss, Auslesewerte vom Rauschen und Signal etc ab. In folgenden Unterabschnitten werden die Grundlagen der eingesetzten Algorithmen erfasst.

\subsection{Schwellenwert-Algorithmus}
Einfach ein threshold einsetzen
