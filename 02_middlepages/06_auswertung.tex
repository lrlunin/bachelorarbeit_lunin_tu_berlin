...
\noindent
Wichtig zu beachten, dass die Bilder, die sich als threscholdene Summe ergeben, haben die fehldetektierten Photonen \gls{fdpa} mit drin.
...
\end{equation}
So im Vergleich mit der Gleichung (\ref{eq:signal_ew}) muss die fdpa von jedem Pixel als Offset abgezogen werden:
\begin{equation}
...
\end{figure}
\noindent
Die experimentellen Daten haben eine größere Breite des Streuringes. Das ist durch die endliche Breite des Strahlprofils zu erklären ist. In dem Sinne ist die Fourier-Transformierte mit einem idealen punktförmigen Strahlprofil gefaltet. Die Maxima der Verteilungen liegen innerhalb der Vertrauensbereiche voneinander.
...
\end{figure}
\noindent
Das Fehlen des charakteristischen Ringmusters weist danach, dass die Photonenfrequenz $h\nu$ sehr nah an der Resonanzfrequenz von Gd liegen soll. Das Fehlen des Musters kann nicht an der zu niedrigeren Photonenfluss liegen, obwohl dieser an der Photonenfrequenz $h\nu_{\text{Gd, Off-Res}}$ tendenziell niedriger als an der $h\nu_{\text{Gd, M5}}$ sein soll. Im Gegensatz dazu ist die Absorptionsstärke der Probe an der Photonenfrequenz $h\nu_{\text{Gd, Off-Res}}$ ist nach der Abb. \ref{fig:rzp_phi_ev} im Vergleich mit $h\nu_{\text{Gd, M5}}$ wesentlich kleiner. Somit ergeben sich die vergleichbare Photonzahlen pro Pixel im Direktstrahl an den beiden Photonenenergien (Abb. \ref{fig:th_50_100_125_150_170_180_200_220_260} und \ref{fig:th_50_100_125_150_170_180_off_resonance}) und betragen ca. \SI{1000}{\photons}.

\subsection{Clustering}
\noindent
Diese Analyse der Gesamtladungsverteilung eines isolierten Photons (Abb. \ref{fig:examples_average_std_5x5_hotspot}) demonstriert die Vorteile des Clustering-Algorithmus-Ansatzes. Unter Berücksichtigung der Ergebnisse dieser Analyse und zwar, dass die Gesamtladung grundsätzlich innerhalb zwei benachbarter Pixel verteilt wurde, wird der $\mathbf{K}_2$ Cluster-Kern benutzt.
\begin{figure}[H]
    \centering
    \input{images/auswertung/cl_2_150_180_200_220_250_290_resonance.pgf}
    \caption{Anzahl von den detektierten Photonen mithilfe des Clustering-Algorithmuses mit Cluster-Kern $\mathbf{K}_2$ und dem Schwellenwert $s_Q$ (a) \SI{150}{\adu}, (b) \SI{180}{\adu}, (c) \SI{200}{\adu}, (d) \SI{220}{\adu}, (e) \SI{250}{\adu} und (f) \SI{290}{\adu}. Aufsummiert werden jeweils \num{50000} Aufnahmen. Aufgenommen an der Gd M5 Resonanzfrequenz.}
    \label{fig:cl_2_0_100_125_150_170_180_resonance}
\end{figure}
\noindent
Die Ergebnisse scheinen auf den ersten Blick gut zu sein, aber es ist nötig, zunächst einen Blick auf die Messdaten an der Photonenenergie $h\nu_\text{Gd, Off-Res}$ zu werfen. \begin{figure}[H]
    \centering
...
!!! PEDESTALS MÜSSEN IMMER MEHR OFFSET mal 4 !!!
Wegen zu hohen Detektorrauschens ist die Anzahl von fehldetektierten Photonen so hoch, dass der Ring an der nicht resonanten frequenz sichtbar ist. Die Standardabweichung eines 2x2 Clusters ist ja 2 mal so hoch im vergleich mit einem einzelnen Pixel. SO muss der Schwellenwert $s_Q$ dementsprechend angepasst werden.

\noindent
Eine sehr große Rolle spielt die regelmäßige Aufnahme der Dunkelbilder, da nun der resultierende Offset eines clusters wird eben 4mal höher.

\noindent
Der Cluster-Kern kann ebenso in Bezug auf die Verteilung, die grundlegend in eine benachbarte pixel, angepasst werden. Zum Beispiel können jeweils die Cluster-Kerne
\begin{equation}
...
\end{bmatrix}
\end{equation}
benutzt werden, um die Standarbweichung von $2\sigma_R$ des Clsuters bis $\sqrt{2}\sigma_R$ zu verringern.
\begin{figure}[H]
    \centering
