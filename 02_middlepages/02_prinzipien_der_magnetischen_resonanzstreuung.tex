...
\label{text:streuung}
Die in dieser Arbeit durchgeführte Experimente basieren auf den Effekten der resonanten Streuung. Die Darstellung der Zusammenhänge orientiert sich auf \cite{kortright_resonant_2013}. 

\noindent
Die resonante Streuung an einem Element wird durch den elementspezifischen atomaren Streufaktor
\begin{align}
f(h\nu) = (\mathbf{\hat{e}_f^*} \cdot \mathbf{\hat{e}_0})f_c - i(\mathbf{\hat{e}_f^*} \times \mathbf{\hat{e}_0})\cdot\mathbf{\hat{m}}f_{m1} + \dots % (\mathbf{\hat{e}_f^*} \cdot \mathbf{\hat{m}})(\mathbf{\hat{e}_0}\cdot\mathbf{\hat{m}})f_{m2} + \dots
...
\noindent
Im Photonenfrequenzbereich $\nu_\text{resonant}$, der nah an der Resonanzabsorptionsfrequenz des Atoms liegt, nehmen die Streukoeffizienten $f_{m1}, f_{m2}$ zu und leisten einen wesentlichen Beitrag zum Gesamtwert des Streufaktors $f$. Es ist auch wichtig zu beachten, dass der Streufaktor $f(h\nu_\text{resonant})$ nun sowohl von den Polarisationsvektoren $\mathbf{\hat{e}_f}$ und $\mathbf{\hat{e}_0}$, als auch von der räumlichen Ausrichtung der Atommagnetisierungsachse des  $\mathbf{\hat{m}}$ in Bezug auf $\mathbf{\hat{e}_f}$ und $\mathbf{\hat{e}_0}$ abhängt.


\noindent
Für diese Arbeit wird insbesondere der Fall betrachtet, 
...
Nach der \emph{Bornsche Näherung} lässt sich die Intensität der Streuung $I$ an den $N$-Atomen als die Summe der Streuung an jedem $n$-ten Atom darstellen. Die Streuung am $n$-ten Atom wird als Produkt dessen Streufaktors $f_{\pm}^n$ mit dem Phasenfaktor $e^{i\mathbf{qr_n}}$ bezeichnet, wobei $\mathbf{q}$ der Streuvektor ist.
\begin{equation}
...
\noindent
Nach Gleichung (\ref{eq:lin_circ_jones}) kann die Intensität der Streuung des linear polarisierten Lichtes $I_{\text{lin}}$ als die Summe
\begin{equation}
...
\end{equation}
bestimmt werden, wobei die Mischterme $\pm2\text{Re}(F_c^*F_m)$ einander ausgleichen. Das Ergebnis, dass die resonante Streuung sowohl für zirkular, als auch für linear polarisierte Strahlung zu beobachten ist, ist insbesondere im Kontext dieser Arbeit wichtig, weil die benutzte Röntgenquelle Strahlung ohne bevorzugte Polarisation emittiert. 
...
\begin{equation}
    ...
    \label{eq:f_m}
\end{equation}
Wird die Streuung unter der Resonanzbedingungen für ein chemisches Element betrachtet, können die Koeffizienten $f_{m1,1/2}$ aller anderen Elemente als vernachlässigbar klein weggelassen werden, wenn diese weit genug voneinander liegen. In dieser Arbeit werden somit die Photonenenergien der Fe L3-Absorptionslinie $h\nu_{\text{Fe, L3}} = \SI{706.97}{\eV}$ und Gd M5-Absorptionslinie $h\nu_{\text{Gd, M5}} = \SI{1184.79}{\eV}$ unabhängig voneinander betrachtet. Die Annahme
\begin{equation}
    f_{m1,1/2}^n = f_{m1,1/2} = \text{konst. } \forall n \in N
\end{equation}
lässt Gleichung (\ref{eq:f_m}) weiter vereinfachen
\begin{equation}
...
\end{equation}
\noindent
Analog zur Proportionalität des Betragsquadrats des Strukturfaktors $\left|F_m(\mathbf{q})\right|^2$ der Fourier-Trans\-for\-mier\-ten der magnetischen Struktur $S_m(\mathbf{q})$ ist der Betragsquadrat des Formfaktors $\left|F_c(\mathbf{q})\right|^2$ proportional zur Fourier-Transformierten des Atomgitters $S_c(\mathbf{q})$. Die magnetische Struktur verändert sich jedoch typischerweise auf \SI{10}{\nano\meter}-\si{\micro\meter} Skala, wobei die typische Bindungslänge innerhalb eines Atomgitters einige Ångström beträgt. So ist der Beitrag von $S_c(\mathbf{q})$ zur gesamten Streuintensität nur bei deutlich größeren Streuvektorenlängen $q$ (größeren Streuwinkeln) zu betrachten.%liegen im Bereich von  sind viel größerordnung, d.h. dass unter der sehr kleinen winkeln (kleine streuvektoren) wird die magnetische streuung betrachet.
